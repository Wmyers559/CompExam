...
main(void)
{
    const uint8_t sBox4[]    = { 0xc, 0x5, 0x6, 0xb, 0x9, 0x0, 0xa, 0xd,
                              0x3, 0xe, 0xf, 0x8, 0x4, 0x7, 0x1, 0x2 };
    const uint8_t invsBox4[] = { 0x5, 0xe, 0xf, 0x8, 0xc, 0x1, 0x2, 0xd,
    ...
    if (eingabe == 0) {
        ...
        do {
            ...
            //	****************** pLayer ******************************
            for (i = 0; i < 8;
                 i++) // clearing of the temporary array temp_pLayer
            {
                temp_pLayer[i] = 0;
            }
            for (i = 0; i < 64; i++) {
                ...
                temp_pLayer[element_destination] |=
                  ((state[element_source] >> bit_source) & 0x1)
                  << bit_destination;
            }
            for (i = 0; i <= 7; i++) {
            ...
            key[9] =
              sBox4[key[9] >> 4] << 4 | (key[9] & 0xF); // S-Box application
    ...
    //	****************** Decryption **************************
        ...
        do {
            ...
            do {
                state[i] =
                  invsBox4[state[i] >> 4] << 4 | invsBox4[state[i] & 0xF];
                i++;
            } while (i <= 7);
            //	****************** End sBox ****************************
            round--;
