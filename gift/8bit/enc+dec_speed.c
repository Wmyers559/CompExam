...
main(void)
{
    ...
    const uint8_t sBox4[] = { 0xc0, 0x50, 0x60, 0xb0, 0x90, 0x00, 0xa0, 0xd0,
                              0x30, 0xe0, 0xf0, 0x80, 0x40, 0x70, 0x10, 0x20 };
    //	8Bit sBox used for the sBox functionality
    const uint8_t sBox8[] = {
        204, 197, 198, 203, 201, 192, 202, 205, 195, 206, 207, 200, 196, 199,
        193, 194, 92,  85,  86,  91,  89,  80,  90,  93,  83,  94,  95,  88,
        84,  87,  81,  82,  108, 101, 102, 107, 105, 96,  106, 109, 99,  110,
        111, 104, 100, 103, 97,  98,  188, 181, 182, 187, 185, 176, 186, 189,
        179, 190, 191, 184, 180, 183, 177, 178, 156, 149, 150, 155, 153, 144,
        154, 157, 147, 158, 159, 152, 148, 151, 145, 146, 12,  5,   6,   11,
        9,   0,   10,  13,  3,   14,  15,  8,   4,   7,   1,   2,   172, 165,
        166, 171, 169, 160, 170, 173, 163, 174, 175, 168, 164, 167, 161, 162,
        220, 213, 214, 219, 217, 208, 218, 221, 211, 222, 223, 216, 212, 215,
        209, 210, 60,  53,  54,  59,  57,  48,  58,  61,  51,  62,  63,  56,
        52,  55,  49,  50,  236, 229, 230, 235, 233, 224, 234, 237, 227, 238,
        239, 232, 228, 231, 225, 226, 252, 245, 246, 251, 249, 240, 250, 253,
        243, 254, 255, 248, 244, 247, 241, 242, 140, 133, 134, 139, 137, 128,
        138, 141, 131, 142, 143, 136, 132, 135, 129, 130, 76,  69,  70,  75,
        73,  64,  74,  77,  67,  78,  79,  72,  68,  71,  65,  66,  124, 117,
        118, 123, 121, 112, 122, 125, 115, 126, 127, 120, 116, 119, 113, 114,
        28,  21,  22,  27,  25,  16,  26,  29,  19,  30,  31,  24,  20,  23,
        17,  18,  44,  37,  38,  43,  41,  32,  42,  45,  35,  46,  47,  40,
        36,  39,  33,  34
    };

    const uint8_t invsBox8[] = {
        85,  94,  95,  88,  92,  81,  82,  93,  91,  84,  86,  83,  80,  87,
        89,  90,  229, 238, 239, 232, 236, 225, 226, 237, 235, 228, 230, 227,
        224, 231, 233, 234, 245, 254, 255, 248, 252, 241, 242, 253, 251, 244,
        246, 243, 240, 247, 249, 250, 133, 142, 143, 136, 140, 129, 130, 141,
        139, 132, 134, 131, 128, 135, 137, 138, 197, 206, 207, 200, 204, 193,
        194, 205, 203, 196, 198, 195, 192, 199, 201, 202, 21,  30,  31,  24,
        28,  17,  18,  29,  27,  20,  22,  19,  16,  23,  25,  26,  37,  46,
        47,  40,  44,  33,  34,  45,  43,  36,  38,  35,  32,  39,  41,  42,
        213, 222, 223, 216, 220, 209, 210, 221, 219, 212, 214, 211, 208, 215,
        217, 218, 181, 190, 191, 184, 188, 177, 178, 189, 187, 180, 182, 179,
        176, 183, 185, 186, 69,  78,  79,  72,  76,  65,  66,  77,  75,  68,
        70,  67,  64,  71,  73,  74,  101, 110, 111, 104, 108, 97,  98,  109,
        107, 100, 102, 99,  96,  103, 105, 106, 53,  62,  63,  56,  60,  49,
        50,  61,  59,  52,  54,  51,  48,  55,  57,  58,  5,   14,  15,  8,
        12,  1,   2,   13,  11,  4,   6,   3,   0,   7,   9,   10,  117, 126,
        127, 120, 124, 113, 114, 125, 123, 116, 118, 115, 112, 119, 121, 122,
        149, 158, 159, 152, 156, 145, 146, 157, 155, 148, 150, 147, 144, 151,
        153, 154, 165, 174, 175, 168, 172, 161, 162, 173, 171, 164, 166, 163,
        160, 167, 169, 170
    };
    ...
    }; // volatile = compiler command to not delete this value while optimizing
       // Counter
    ...
    for (round = 0; round <= 31; round++) {
        ...
        key[9] =
          sBox4[key[9] >> 4] | (key[9] & 15); // sBox4 applied on Bits 79-76

        if ((round + 1) % 2 == 1)
            key[1] ^= 128;
        key[2] = ((((round + 1) >> 1) ^ (key[2] & 15)) |
                  (key[2] & 240)); // Salt (XOR with roundcounter)
    ...
    if (eingabe == 1) {
        ...
        do         // round loop
        {
            ...
            state[0] =
              sBox8[state[0] ^ subkey[round][0]]; // 8Bit sBox lookup + key XOR
            ...
            asm volatile("clr %0				\n"
                         ...
                         : "r"(state[0]),
                           ...
                           "r"(state[7]));
            state[0] =
              temp_pLayer[0]; // Store temporary values in state variable
        ...
        do {
            ...
            asm volatile("clr %0				\n"
                         ...
                         : "r"(state[0]),
                           ...
                           "r"(state[7]));
            state[0] =
              temp_pLayer[0]; // Store temporary values in state state variable
            state[1] =
              temp_pLayer[1]; // Store temporary values in state state variable
            state[2] =
              temp_pLayer[2]; // Store temporary values in state state variable
            state[3] =
              temp_pLayer[3]; // Store temporary values in state state variable
            state[4] =
              temp_pLayer[4]; // Store temporary values in state state variable
            state[5] =
              temp_pLayer[5]; // Store temporary values in state state variable
            state[6] =
              temp_pLayer[6]; // Store temporary values in state state variable
            state[7] =
              temp_pLayer[7]; // Store temporary values in state state variable
            //	****************** End pLayer **************************
            state[0] =
              invsBox8[state[0]] ^ subkey[k][0]; // 8Bit sBox lookup + key XOR
            state[1] =
              invsBox8[state[1]] ^ subkey[k][1]; // 8Bit sBox lookup + key XOR
            state[2] =
              invsBox8[state[2]] ^ subkey[k][2]; // 8Bit sBox lookup + key XOR
            state[3] =
              invsBox8[state[3]] ^ subkey[k][3]; // 8Bit sBox lookup + key XOR
            state[4] =
              invsBox8[state[4]] ^ subkey[k][4]; // 8Bit sBox lookup + key XOR
            state[5] =
              invsBox8[state[5]] ^ subkey[k][5]; // 8Bit sBox lookup + key XOR
            state[6] =
              invsBox8[state[6]] ^ subkey[k][6]; // 8Bit sBox lookup + key XOR
            state[7] =
              invsBox8[state[7]] ^ subkey[k][7]; // 8Bit sBox lookup + key XOR
