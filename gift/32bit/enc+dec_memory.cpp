...
main(void)
{
    ...
    if (eingabe == 0) {
        ...
        for (i = 0; i < 31; i++) {
            ...
            for (sBoxNr = 0; sBoxNr < 16; sBoxNr++) {
                ...
                state |=
                  sBox4[sBoxValue]; // put new value to lowest nibble (sBox)
            ...
            for (int k = 0; k < 64; k++) {
                int position =
                  (16 * k) % 63; // Arithmentic calculation of the p-Layer
    ...
    //	****************** End Encryption **********************
        ...
        for (i = 0; i < 31; i++) {
            ...
            for (int k = 0; k < 64; k++) {
                int position =
                  (4 * k) % 63; // Arithmentic calculation of the p-Layer
                if (k == 63)    // Exception for bit 63
                    position = 63;
                temp |= ((state >> k) & 0x1) << position; // result writing
            }
            ...
            for (sBoxNr = 0; sBoxNr < 16; sBoxNr++) {
                ...
                state &= 0xFFFFFFFFFFFFFFF0; // kill lowest nibble
                state |=
                  invsBox4[sBoxValue]; // put new value to lowest nibble (sbox)
                state = rotate4l_64(state); // next(rotate by one nibble)
            }
            //	****************** End sBoxLayer ***********************
        }
